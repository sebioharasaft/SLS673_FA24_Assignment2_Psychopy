...
expName = 'SLS673_FA24_Assignment2'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/sebi/Library/CloudStorage/OneDrive-hawaii.edu/Fall 2024/SLS673 - Applied Psycholinguistics & SLA/Assignments/Assignment 2/SLS673_FA24_Assignment2_Psychopy/SLS673_FA24_Assignment2_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions_text = visual.TextStim(win=win, name='instructions_text',
        text='Welcome to our experiment!\n\nYou will see a sentence on the screen. Please read the sentence out loud. Once you have done so, press the space button to move on.\n\nNow, press the space button to see sentences.',
        font='Arial',
    ...
    # --- Initialize components for Routine "last_word_response" ---
    recall_text = visual.TextStim(win=win, name='recall_text',
        text='Now recall the last words in the sentences you just read.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    Instructions = data.Routine(
        name='Instructions',
        components=[instructions_text, key_resp_2],
    )
    ...
    # --- Prepare to start Routine "last_word_response" ---
    # create an object to store info about Routine last_word_response
    last_word_response = data.Routine(
        name='last_word_response',
        components=[recall_text, textbox_1, textbox_2],
    )
    last_word_response.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    textbox_1.reset()
    textbox_2.reset()
    # store start times for last_word_response
    last_word_response.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    last_word_response.tStart = globalClock.getTime(format='float')
    last_word_response.status = STARTED
    thisExp.addData('last_word_response.started', last_word_response.tStart)
    last_word_response.maxDuration = None
    # keep track of which components have finished
    last_word_responseComponents = last_word_response.components
    for thisComponent in last_word_response.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "last_word_response" ---
    last_word_response.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            last_word_response.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in last_word_response.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "last_word_response" ---
    for thisComponent in last_word_response.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for last_word_response
    last_word_response.tStop = globalClock.getTime(format='float')
    last_word_response.tStopRefresh = tThisFlipGlobal
    thisExp.addData('last_word_response.stopped', last_word_response.tStop)
    thisExp.nextEntry()
    # the Routine "last_word_response" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
